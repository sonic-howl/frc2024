...
class Launchers:
  def __init__(self):
    ...
    self.feedswitch = wpilib.DigitalInput(3)
  ...
  def shoot(self, fire: float):
    if fire <= 0.05:
      ...
      if (
        self.encoder1.getVelocity() < -4500.0
        and self.encoder2.getVelocity() > 4500.0
        and self.lowerswitch.get()
      ):
        self.feed.set(1.0)
      else:
        self.feed.set(0.0)
